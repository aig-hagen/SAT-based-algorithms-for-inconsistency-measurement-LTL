...
/* Fahiem Bacchus changes start. */
#include <algorithm>
#include <iostream>
#include <numeric>
/* Fahiem Bacchus changes end. */
...
struct External {
  ...
  unsigned e_vlit(int lit) { return (lit < 0) + 2u * (unsigned)vidx(lit); }
  int e_u2i(unsigned u) {
    int res = u / 2;
    assert(res <= max_var);
    if (u & 1) res = -res;
    return res;
  ...
  int eqLit(int l) {
    auto e{l};
    while(e_vlit(e) < eqmap.size() && eqmap[e_vlit(e)] != e)
      e = eqmap[e_vlit(e)];
    if(e != l)
      //e_vlit(l) out of range ==> e == l, so this is safe
      eqmap[e_vlit(l)] = e;
    return e;
  }
  
  void set_eqLit(int l, int e) {
    l = eqLit(l);
    e = eqLit(e);
    if(l == e) return;
    //set 0 bit to one---ensures eqmap large enough to cover both l and -l
    ensure_size_eqmap(e_vlit(l) | 1u);
    eqmap[e_vlit(l)] = e;
    eqmap[e_vlit(-l)] = -e;
  }

  void ensure_size_eqmap(unsigned sz) {
    auto cur_sz = eqmap.size();
    if (cur_sz > sz) return;
    eqmap.resize(sz+1);
    for(size_t i = cur_sz; i < sz + 1; ++i)
      eqmap[i] = e_u2i(i);
  }
