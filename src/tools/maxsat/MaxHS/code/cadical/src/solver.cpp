...
bool Solver::set(const char* arg, int val) {
  ...
  if (strcmp(arg, "log") && strcmp(arg, "quiet") && strcmp(arg, "report") &&
      ...
      strcmp(arg, "verbose") && strcmp(arg, "elimrounds") &&
      strcmp(arg, "vivify")) {
...
int Solver::no_elim_simplify() {
  TRACE("no_elim_simplify");
  REQUIRE_READY_STATE();
  const int res = call_external_no_elim_simplify();
  LOG_API_CALL_RETURNS("no_elim_simpify",  res);
  return res;
}

int Solver::call_external_no_elim_simplify() {
  transition_to_unknown_state();
  assert(state() & READY);
  STATE(SOLVING);
  const int res = external->no_elim_simplify();
  if (res == 10)
    STATE(SATISFIED);
  else if (res == 20)
    STATE(UNSATISFIED);
...
int Solver::eqLit(int l) {
  return external->eqLit(l);
}
...
bool Solver::traverse_clauses(ClauseIterator& it) const {
  ...
  bool res = internal->traverse_clauses(it);
