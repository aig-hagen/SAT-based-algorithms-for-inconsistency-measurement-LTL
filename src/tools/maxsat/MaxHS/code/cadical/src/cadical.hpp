...
class Solver {
 ...
 public:
  ...
  const std::vector<int>& solver_units() const; //return ref to units found by solver
  int eqlit(int);                  // return equivalent literal
  int no_elim_simplify();           // special simplify what does not do variable elimination
  int call_external_no_elim_simplify();
  /* Fahiem Bacchus changes end. */
  ...
  bool get_ith_clause(std::vector<int>& cls, size_t i) const;
  /* Fahiem Bacchus changes end. */
